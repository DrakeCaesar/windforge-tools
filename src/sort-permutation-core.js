...
const PLACEABLE_SETUP_STAT_TYPE_SET = new Set(
  PLACEABLE_SETUP_STAT_OBJECT_TYPES,
);
...
function createSortPermutationBindings(deps) {
  ...
  function componentSellPrice(item, wisdomOverride) {
    const recipes =
      catalog().recipesByProduct && catalog().recipesByProduct[item.name];
    ...
    for (let i = 0; i < recipes.length; i++) {
      ...
      const outQty =
        rec.craftQuantity != null && typeof rec.craftQuantity === "number"
          ? rec.craftQuantity
          : 1;
      ...
      for (let j = 0; j < ings.length; j++) {
        ...
        const qty =
          ing.quantity != null && typeof ing.quantity === "number"
            ? ing.quantity
            : 1;
        sum += sp * qty;
      }
  ...
  function getSortValue(item, colId, wisdomForPrices) {
    ...
    if (colId === "componentSell")
      return componentSellPrice(item, wisdomForPrices);
    ...
    if (colId === "dmgPhysical") {
      ...
      return d && typeof d.physicalDamage === "number"
        ? d.physicalDamage
        : null;
    }
    if (colId === "meleeTimeBetweenAttacks") {
      const m = getMeleeWeaponSetup(item);
      return m && typeof m.timeBetweenAttacks === "number"
        ? m.timeBetweenAttacks
        : null;
    }
    ...
    if (colId === "dmgKnockback") {
      const d = getMeleeDamageDesc(item);
      return d && typeof d.knockbackMagnitude === "number"
        ? d.knockbackMagnitude
        : null;
    }
  ...
  function collectSortCacheJobSpecs(currentWisdom) {
    ...
    for (var ci = 0; ci < COLUMNS.length; ci++) {
      ...
      for (var d = 0; d < 2; d++) {
        ...
        for (var s = 0; s < 2; s++) {
          var secondary =
            s === 0 ? SECONDARY_SORT_INTERNAL_NAME : SECONDARY_SORT_RECIPE_BASE;
  ...
  function applyMatricesFromWorkerBuffers(
    n,
    buyBuf,
    sellBuf,
    compBuf,
